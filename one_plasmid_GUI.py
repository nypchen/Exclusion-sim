...
from one_plasmid_core import run_simulation, plot_data, plot_movie
import os, re, string
import tkinter
...
def verify_movie_condition():
    if input_dict["Repeats (n)"] != 1 or len(input_dict["Exclusion index (EI)"]) != 1:
...
def default_filename(parent_path=os.getcwd()+'/'):
    ...
    i = 0
    while os.path.exists(parent_path + f"Saved_data_1P_{current_date}{'' if i==0 else '_'+str(i)}.pkl"):
        i += 1
        if i > 100:
            raise FileExistsError("Too many saved files in the selected directory")

    return f"Saved_data_1P_{current_date}{'' if i==0 else '_'+str(i)}.pkl"
...
def refresh_pkl_files():
    ...
    for file in current_dir_pkl_files:
        ...
        if data_dict['run_type'] != 'one_plasmid':
            continue
...
def browse_saved_data(event):
    ...
    if filesize > max_size_allowed:
        override_file_size = tkinter.messagebox.askyesno(f"The file you selected is {filesize // (1024*1024)} MB. Loading it may take up a significant amount of your computer resources. Continue anyways?")
...
def load_data():
    ...
    if list(data_dict.keys()) != ['run_type', 'run_parameters', 'percentages', 'frames']:
        show_load_data_warning_message("Error: invalid file selected.")
        return 0

    if data_dict['run_type'] != 'one_plasmid':
        show_load_data_warning_message(f"Error: wrong run type ({data_dict['run_type']}) selected!")
        return 0

    if len(data_dict['run_parameters']) != 7:
        show_load_data_warning_message("Error: invalid file selected.")
        return 0
    else:
        show_load_data_warning_message("")
...
def disp_data_parameters(data_dict):
    ...
    N1, n1, d1, EI_values1, max_steps1, show_movie1, save_path1 = data_dict['run_parameters']
    ...
    params_output += f"Exclusion index (EI): {str(EI_values1).strip('[]')}\n"
    ...
    reload_params_button.bind("<Button-1>", command=lambda event, N=N1, n=n1, d=d1, EI=EI_values1, max_steps=max_steps1, show_movie=show_movie1, save_path=save_path1: reload_params(event, N, n, d, EI, max_steps, show_movie, save_path))


def reload_params(event, N, n, d, EI, max_steps, show_movie, save_path):
    tabview.set("Run")
    loaded_params = (N, n, d, EI, max_steps)
    ...
    for i, param in enumerate(loaded_params):
        ...
        if i != 3:
            ...
            if len(EI) == 1:
                EI_formatted = str(EI[0]).strip('[]')
            elif len(EI) >= 2:
                EI_formatted = str(EI).strip('[]')
    ...
    input_dict['Exclusion index (EI)'] = EI
...
main_title = ctk.CTkLabel(app, text="One plasmid simulation", font=title_font)
...
# Initialize values
input_keys = ["Array size (N)", "Repeats (n)", "Dead cutoff (d)", "Exclusion index (EI)", "Max steps (l)", "Save path", "Filename"]
input_values_default = [100, 250, 5, [1, 5, 100], 500, os.getcwd()+'/', default_filename()]
...
for i, label_text in enumerate(input_dict.keys()):
    if i not in (5, 6): # Not the checkboxes, only the input fields
